...
#define foreach_stdlist(type, name, container) for (typename std::list<type>::iterator name = (container).begin(); name != (container).end(); ++name)
/// @brief Provides a simpler syntax to iterate through an std::vector.
#define foreach_stdvector(type, name, container) for (typename std::vector<type>::iterator name = (container).begin(); name != (container).end(); ++name)
/// @brief Provides a simpler syntax to iterate through an std::map.
#define foreach_stdmap(type_key, type_value, name, container) for (typename std::map<type_key, type_value>::iterator name = container.begin(); name != container.end(); ++name)
